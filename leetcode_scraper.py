...
class LeetcodeScraper:
    ...
    def get_proxy(self):
        ...
        data = {
            'format': 'text',
            'protocol': ['https'],
            "apiKey": os.getenv('PROXIFLY_API_KEY')
        }

        try:
            response = requests.post(url, json=data, headers=headers)
            proxies = response.text.split('\n')
            self.proxy = proxies[0]  # Assuming you want to use the first proxy
    ...
    def scrape_all_global_ranking_users_selection(self, start_page, end_page):
        final_response = []
